...
describe('Composant API:', function () {
  ...
  describe('GET /api/composants', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).get('/api/composants').expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        composants = res.body;
  ...
  describe('POST /api/composants', function () {
    beforeEach(function (done) {
      (0, _supertest2.default)(app).post('/api/composants').send({
        titre: 'led verte',
        type: 'led',
  ...
  describe('GET /api/composants/:id', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).get('/api/composants/' + newComposant._id).expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        composant = res.body;
  ...
  describe('PUT /api/composants/:id', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).put('/api/composants/' + newComposant._id).send({
  ...
  describe('DELETE /api/composants/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      (0, _supertest2.default)(app).delete('/api/composants/' + newComposant._id).expect(204).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when composant does not exist', function (done) {
      (0, _supertest2.default)(app).delete('/api/composants/' + newComposant._id).expect(404).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
...
//# sourceMappingURL=composant.integration.js.map
