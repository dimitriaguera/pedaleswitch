...
describe('Cache API:', function () {
  ...
  describe('GET /api/caches', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).get('/api/caches').expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        caches = res.body;
  ...
  describe('POST /api/caches', function () {
    beforeEach(function (done) {
      (0, _supertest2.default)(app).post('/api/caches').send({
        name: 'New Cache',
  ...
  describe('GET /api/caches/:id', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).get('/api/caches/' + newCache._id).expect(200).expect('Content-Type', /json/).end(function (err, res) {
        if (err) {
          return done(err);
        }
        cache = res.body;
  ...
  describe('PUT /api/caches/:id', function () {
    ...
    beforeEach(function (done) {
      (0, _supertest2.default)(app).put('/api/caches/' + newCache._id).send({
        name: 'Updated Cache',
  ...
  describe('DELETE /api/caches/:id', function () {
    ...
    it('should respond with 204 on successful removal', function (done) {
      (0, _supertest2.default)(app).delete('/api/caches/' + newCache._id).expect(204).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
    ...
    it('should respond with 404 when cache does not exist', function (done) {
      (0, _supertest2.default)(app).delete('/api/caches/' + newCache._id).expect(404).end(function (err, res) {
        if (err) {
          return done(err);
        }
        done();
